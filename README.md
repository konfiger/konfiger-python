...
In KeyValueDB the key value pair is stored in `[KeyValueObject...]` type, all search, 
...
`__str__` method also parse the `keyValueObjects` content into a valid string with regards to the 
